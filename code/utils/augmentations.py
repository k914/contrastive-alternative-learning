...
def preprocess(img, img2, bbox_labels, mode, image_path):
    ...
    img2 = img2.resize((cfg.resize_width, cfg.resize_height),
                     resample=interp_mode[interp_indx])
    ...
    if mode == 'train':
        ...
        if mirror == 1:
            img = img[:, ::-1, :]
            img2 = img2[:, ::-1, :]
            # a = six.moves.xrange(len(sampled_labels))
    ...
    return img, img2, sampled_labels
